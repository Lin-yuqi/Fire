...
base::Status encode_prompt(const token::LlamaTokenizer& tokenizer,
    ...
    auto status = append_encoded(
        tokenizer, "<|system|>\n" + std::string(kSystemPrompt), true, output);
    if (!status.ok()) {
        return status;
    }
    ...
    for (const auto& turn : history) {
        status = append_encoded(tokenizer, "\n<|user|>\n" + turn.user, false, output);
        if (!status.ok()) {
            return status;
        }
        output.push_back(tokenizer.eos_id());
        ...
        status =
            append_encoded(tokenizer, "\n<|assistant|>\n" + turn.assistant, false, output);
        if (!status.ok()) {
            return status;
        }
        output.push_back(tokenizer.eos_id());
...
base::Status generate_response(model::TinyLlamaModel& model,
                               ...
                               sampler::ArgmaxSampler& sampler,
                               const std::vector<int32_t>& prompt_ids,
                               tensor::Tensor& logits, const op::OpContext& context,
                               std::string& response) {
    auto status = model.reset(context);
    if (!status.ok()) {
        return status;
    }

    int32_t position = 0;
    for (const int32_t token_id : prompt_ids) {
        status = model.forward(token_id, position, logits, context);
        if (!status.ok()) {
            return status;
        }
        ++position;
    ...
    generated_ids.reserve(MaxNewTokens);
    std::string printed_response;

    for (size_t step = 0; step < MaxNewTokens && position < model.config().max_seq_len;
         ++step) {
        ...
        std::string decoded;
        status = tokenizer.decode(generated_ids, decoded);
        if (!status.ok()) {
            return status;
        }
    ...
    status = tokenizer.decode(generated_ids, response);
    if (!status.ok()) {
        return status;
    }
...
int run_chat(const std::string& model_path, const std::string& tokenizer_path,
    ...
    const size_t prompt_budget =
        static_cast<size_t>(model->config().max_seq_len) - MaxNewTokens;
    ...
    while (true) {
        ...
        if (user_input == "/help") {
            std::cout << "/reset clears conversation history; /exit quits.\n";
            continue;
        }
        if (user_input == "/reset") {
            history.clear();
        ...
        size_t dropped_turns = 0;
        while (true) {
            status = encode_prompt(tokenizer, history, user_input, prompt_ids);
            if (!status.ok()) {
                return report_error("encoding prompt", status);
            }
            if (prompt_ids.size() <= prompt_budget) {
                break;
            }
            if (history.empty()) {
                prompt_ids.clear();
        ...
        if (prompt_ids.empty()) {
            std::cerr << "Input is too long for the TinyLlama context window.\n";
            continue;
        }
        if (dropped_turns != 0) {
            std::cout << "[context: dropped " << dropped_turns << " oldest turn(s)]\n";
        }

        std::cout << "Assistant> " << std::flush;
        std::string response;
        status = generate_response(*model, tokenizer, sampler, prompt_ids, logits, context,
                                   response);
        std::cout << '\n';
        if (!status.ok()) {
